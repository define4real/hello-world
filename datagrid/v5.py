...
def xxcheck_h_scroll_end(module, hwnd):
...
def check_h_scroll_end(tt):
    ...
    win32api.SendMessage(tt.v_hwnd, win32con.WM_HSCROLL, win32con.SB_PAGERIGHT, None)
    time.sleep(0.5)
    ...
    if x1 == x2 and w1 == w2 and v1 == v2 and type1 == type2:
        return True
...
class GetTableTest:
    def __init__(self, hwnd=None, title_name=None, table_name=None):
        ...
        self.head_text_list = []
        self.head_left_list = []
        self.head_right_list = []
        self.head_scroll_list = []
        self.head_index = -1
        self.head_x_list = []
    ...
    def add_data_to_list(self, tmp_list, line_num):
        if len(tmp_list) <= 0:
            return
        if self.first_line_same_with_header_flag and line_num == 0:
        ...
        if add_flag:
            if self.h_scroll_count > 0:
                if (line_num == 1):
                    print("xxx data_list=", len(self.data_list))
                    print("xxx line_num=", line_num)
                    print("xxx self.page_line_count=", self.page_line_count)
                    print("xxx line_num - self.page_line_count - 1=", line_num - self.page_line_count - 1)
                    print("xxx self.page_line_count=", self.data_list[line_num - self.page_line_count - 1])
                    print("tmp_list=", tmp_list)
                    

                self.data_list[line_num - self.page_line_count - 1] += tmp_list  # idx -1?
                if (line_num == 1):
                    print("xxx self.page_line_count=", self.data_list[line_num - self.page_line_count - 1])
            else:
                print("yyy data_list=", len(self.data_list))
                print("yyy line_num=", line_num)
                print("yyy self.page_line_count=", self.page_line_count)
                self.data_list.append(tmp_list)

        # if (line_num == 1):
            # print("data_list【0】=", self.data_list[0])
            # print("add_flag =", add_flag)
            # print("tmp_list=", tmp_list)
            # print("line_num = ", line_num)
            # print("page_line_count = ", self.page_line_count)
            # print("v_scroll_count = ", self.v_scroll_count)
    ...
    def get_header_info(self, cell_child):
        ...
        if self.v_scroll_count != 0 or self.line_num > 1:
            return
        ...
        left = cell_child.BoundingRectangle.left  # left
        head_x = right = cell_child.BoundingRectangle.right  # left
        if self.h_scroll_count == 0:
            if head_text not in self.head_text_list:
                self.head_text_list.append(head_text)
                self.head_left_list.append(left)
                self.head_right_list.append(right)
                self.head_scroll_list.append(self.head_index)
                self.head_x_list.append(head_x)  # 滚动之前的表头对应x，
                print("head_x1=", head_x)
                print("head_text1=", head_text)
            else:
                # 第一页的表头有一样的，但是位置不同
                idx = self.head_text_list.index(head_text)
                if idx >= 0 and self.head_x_list[idx] != head_x:
                    self.head_text_list.append(head_text)
                    self.head_left_list.append(left)
                    self.head_right_list.append(right)
                    self.head_scroll_list.append(self.head_index)
                    self.head_x_list.append(head_x)  # 滚动之前的表头对应x，
                    print("head_x6=", head_x)
                    print("head_text6=", head_text)
        else:
            ...
            if head_text not in self.head_text_list:  # 依赖表头的唯一性
                ...
                # 这个条件有可能不成立，最后一屏，会跳过去

                if head_x in self.head_x_list:
                    idx = self.head_x_list.index(head_x)
                    left_x =  self.head_left_list[idx]
                    if left_x == left and (not head_x in self.exclude_head_x_list):
                        self.exclude_head_x_list.append(head_x)
                    # self.skip_y = cellChild.BoundingRectangle.bottom
                    # print("head_x3=", head_x)
                    # print("head_text3=", head_text)
                    return

                # 依赖表头的唯一性 and idx < len(self.head_x_list)
                # and self.head_index == len(self.head_x_list)
                
                #print("len(self.exclude_head_x_list)=", len(self.exclude_head_x_list))
                if len(self.exclude_head_x_list) == self.head_index and head_text == self.last_header_text:
                    self.skip_first_col_list[self.h_scroll_count] = 1
                    print("head_x4=", head_x)
                    print("head_text4=", head_text)
                    return

                #print("head_x9=", head_x)
                #print("head_text9=", head_text)
                
                # idx = self.head_text_list.index(head_text) # last_index bug !!!!
                self.head_text_list.append(head_text)
                self.head_left_list.append(left)
                self.head_right_list.append(right)
                self.head_scroll_list.append(self.head_index)
    ...
    def check_xx_end(self):
        if self.last_header_element:
            if self.header_use_name_or_value_flag == 1:
                name_value = self.last_header_element.GetLegacyIAccessiblePattern().Value
            else:
                name_value = self.last_header_element.Name  # GetLegacyIAccessiblePattern().Value
            left = self.last_header_element.BoundingRectangle.left
            right = self.last_header_element.BoundingRectangle.right
            print("name_value=", name_value)
            print("elf.last_header_text =", self.last_header_text )
            if name_value == self.last_header_text and left == self.last_header_left and right == self.last_header_right:
                self.is_h_scroll_end = 2
                #self.cell_child = None

    def get_horizontal_scroll_cell(self):
        if self.header_count == 0:  # 说明不需要水平滚动
            print("说明不需要水平滚动")
            self.is_h_scroll_end = 1
            self.cell_child = None
            return

        # if self.check_h_scroll_end:
            # self.check_h_scroll_end = False
            # print("last_header_element name=", self.last_header_element.Name)
            # print("last_header_text name=", self.last_header_text)
        ...
        if self.cell_child.BoundingRectangle.right in self.exclude_head_x_list:
            # 水平滚动之后,冻结的列，要排除
            pass
        else:
            # 第一个不是表头的表头
        ...
        while self.cell_child:
            ...
            if self.cell_child.ControlTypeName == "DataItemControl" or \
                ...
                if self.h_scroll_count == 0:
                    self.tmp_list.append(self.cell_child.GetLegacyIAccessiblePattern().Value)
                    # if self.line_num == 1:
                        # print("h_scroll_count == 0")
                        # print(self.cell_child.GetLegacyIAccessiblePattern().Value)
    ...
    def get_data(self):
        ...
        while self.line_child:
            ...
            if self.is_h_scroll_end == 1:
                return

            if self.v_scroll_count == 0 and self.find_item:
                # 这个最后一次加上去的有可能会重复,所以要记录添加的个数
                self.add_col_count = len(self.tmp_list)
                print("self.add_col_count=", self.add_col_count)

            if len(self.data_list) == 0 and self.find_item and self.check_header_equal_row0(self.tmp_list):
                self.first_line_same_with_header_flag = True
                print("self.first_line_same_with_header_flag tmp_list =", self.tmp_list)
...
def run_test():
    ...
    tt = GetTableTest(hwnd=0x3211E8)  # 0x2059E 0x270684
    tt.init_values()
    ...
    while True:
        ...
        if check_v_scroll_end(tt):
            ...
            tt.head_index = -1
            if tt.page_line_count == 0:
                tt.page_line_count = len(tt.data_list)
            else:
                tt.page_line_count = len(tt.data_list) - tt.page_line_count
    ...
    if tt.header_count != 0 and len(tt.data_list) > 0:
        ...
        des = src + diff
        if diff > 0:
            # delete_last_duplicate_col(tt, src, des)
            print("\n\n\n=========重复的列数据")
