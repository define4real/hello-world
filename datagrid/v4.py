...
class GetTableTest:
    ...
    def get_header_info(self, cell_child):
        ...
        if self.h_scroll_count == 0:
            ...
            if head_text not in self.head_text_list:  # 依赖表头的唯一性
                ...
                self.head_left_list.append(left)
                self.head_right_list.append(right)
                self.head_scroll_list.append(tt.head_index)
                self.header_count += 1
            else:

                # if idx >= len(self.head_x_list):  #后面翻页出现重复的
                # self.head_text_list.append(head_text)
                # self.head_left_list.append(left)
                # self.head_right_list.append(right)
                # print("head_x777=", head_x)
                # print("head_text777=", head_text)
                # return
                ...
                print("len(self.head_x_list)=", len(self.head_x_list))
                ...
                # if self.head_scroll_list[idx] != self.h_scroll_count or (self.head_left_list[idx] != left and self.head_right_list[idx] != right):
                self.head_text_list.append(head_text)
                self.head_left_list.append(left)
                self.head_right_list.append(right)
                self.head_scroll_list.append(self.head_index)
    ...
    def get_horizontal_scroll_cell(self):
        ...
        if self.cell_child.BoundingRectangle.right in self.exclude_head_x_list:
            ...
            # if self.line_num == 1:
            #     print("self.is_first_span_col=", self.is_first_span_col)
            #     print("self.self.skip_first_col_list=", self.skip_first_col_list)
    ...
    def get_data(self):
        ...
        if len(self.head_text_list) > 0:  # and not self.last_header_text
            ...
            self.last_header_left = self.head_left_list[-1]
            self.last_header_right = self.head_right_list[-1]
...
def run_test():
    ...
    tt = GetTableTest(hwnd=0x401402)  # 0x2059E 0x270684
    tt.init_values()
    ...
    while True:
        ...
        if check_v_scroll_end(tt):
            ...
            if tt.page_line_count == 0:
                self.page_line_count = len(tt.data_list)
            else:
                tt.page_line_count = len(tt.data_list) - self.page_line_count
